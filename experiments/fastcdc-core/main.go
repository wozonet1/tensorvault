...
// 针对 AI 大文件场景的配置 (单位: KB)
const (
	MinSize   = 4 * 1024  // 4KB: 最小块大小 (跳过区域)
	AvgSize   = 8 * 1024  // 8KB: 期望平均大小
	MaxSize   = 64 * 1024 // 64KB: 最大块大小
	NormLevel = 2         // 归一化等级 (1, 2, 3)
)

// -----------------------------------------------------------------------------
// 2. 核心算法: Gear Hash Table (复制自 fastcdc-go)
// -----------------------------------------------------------------------------
// 这是一个预计算好的随机整数表，用于将 byte 映射为 uint64
// 来自https://github.com/jotfs/fastcdc-go
// License: Apache 2.0
...
// -----------------------------------------------------------------------------
// 3. 核心逻辑: FastCDC 切分器 ( The Engine)
// -----------------------------------------------------------------------------

// Cut 将 data 切分成多个 chunk 的 offset 切点
func Cut(data []byte) []int {
	...
	for offset < len(data) {
		// 剩余数据不够最小块，直接作为最后一块
		if len(data)-offset <= MinSize {
			cutPoints = append(cutPoints, len(data))
			break
		}

		// A. 跳过最小块 (Sub-minimum Skipping)
		// 这是 FastCDC 速度快的第一个原因：前 MinSize 字节直接不看
		fp := uint64(0)
		idx := offset + MinSize
...
// -----------------------------------------------------------------------------
// 4. 验证实验: CDC vs Fixed (Demonstration)
// -----------------------------------------------------------------------------
...
func main() {
	...
	fmt.Println("=== Experiment: FastCDC Robustness ===")
	fmt.Printf("Data A size: %d bytes\n", len(dataA))
	fmt.Printf("Data B size: %d bytes (Inserted 5 bytes at head)\n\n", len(dataB))
	...
	// 打印结果对比
	fmt.Printf("Data A chunks: %d\n", len(cutsA))
	fmt.Printf("Data B chunks: %d\n", len(cutsB))

	// 验证偏移量
	// 如果是 CDC，Data B 的第二个块长度应该和 Data A 的第一个块长度一致 (只要没在第一个块被切断)
	// 或者更直观地，我们看最后几个切点的位置差

	lastCutA := cutsA[len(cutsA)-2] // 倒数第二个切点
	...
	if lastCutB-lastCutA == 5 {
		...
		fmt.Println("\n❌ FAILURE. Boundaries did not align.")
	}
}
